...
static void process_def(cbm_pipeline_ctx_t *ctx, const CBMDefinition *def, const char *rel) {
     ...
     * field resolution), Variable/Field (READS/WRITES resolution), EnumCase
     * (Swift/Kotlin enum-case READS — M2-c, pro fork), and Table/View (SQL
     * FROM/JOIN lineage). pass_parallel.c and pipeline_incremental.c seed
